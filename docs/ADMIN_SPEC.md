...
## 13. ניהול פלטפורמה (סופר אדמין)

### 13.1 הגדרות מחירים דינמיות

הפלטפורמה תומכת במחירים דינמיים הניתנים לעריכה על ידי הסופר אדמין דרך `/admin/settings`.

#### טבלאות בסכמה

**platform_settings (key-value):**
| key | תיאור | ברירת מחדל |
|-----|-------|------------|
| `branding_plan_price` | מחיר מנוי מסלול תדמית | ₪299 |
| `quickshop_plan_price` | מחיר מנוי קוויק שופ | ₪399 |
| `trial_days` | ימי נסיון | 7 |
| `transaction_fee_rate` | אחוז עמלה מעסקאות | 0.5% |
| `vat_rate` | אחוז מע"מ | 18% |
| `min_charge_amount` | סכום מינימלי לחיוב | ₪1 |

**plugin_pricing:**
| שדה | תיאור |
|-----|-------|
| `plugin_slug` | מזהה התוסף |
| `monthly_price` | מחיר חודשי |
| `trial_days` | ימי נסיון לתוסף |
| `is_active` | האם פעיל |

**סטטוס:** ✅ מושלם

#### דף ניהול הגדרות פלטפורמה
...
│  עמלות                                                      │
│  ┌─────────────────────────────────────────────────────────┐│
│  │ עמלת עסקאות               סכום מינימלי לחיוב           ││
│  │ ┌──────────────┐           ┌──────────────┐             ││
│  │ │ 0.5%         │           │ ₪1           │             ││
│  │ └──────────────┘           └──────────────┘             ││
│  └─────────────────────────────────────────────────────────┘│
...
│  מחירי תוספים                                               │
│  ┌─────────────────────────────────────────────────────────┐│
│  │ תוסף                    מחיר חודשי    ימי נסיון        ││
│  │────────────────────────────────────────────────────────│ │
│  │ ביקורות מוצרים         ₪29          14                │ │
│  │ התראות מלאי             ₪19          14                │ │
│  │ SMS עדכוני הזמנה        ₪49          7                 │ │
│  │ יועץ מוצרים AI          ₪79          14                │ │
│  │ שחזור עגלות נטושות      ₪49          14                │ │
│  │ רשימת משאלות            ₪19          14                │ │
│  │ הערות להזמנה            ₪0           14                │ │
│  │ מוצרים דיגיטליים        ₪39          14                │ │
│  │ חבילות מוצרים           ₪29          14                │ │
│  └─────────────────────────────────────────────────────────┘│
│                                                             │
│                                                    [שמור]  │
...
| קובץ | תיאור |
|------|-------|
| `src/lib/billing/platform-settings.ts` | שירות הגדרות פלטפורמה עם cache |
| `src/lib/billing/payplus-billing.ts` | אינטגרציית PayPlus עם מחירים דינמיים |
| `src/lib/billing/billing-service.ts` | שירות חיוב (מנויים, עמלות, תוספים) |
| `src/app/admin/settings/page.tsx` | דף הגדרות סופר אדמין |
| `src/app/admin/settings/settings-form.tsx` | טופס הגדרות פלטפורמה |
| `src/app/admin/settings/plugin-pricing-form.tsx` | טופס מחירי תוספים |
| `src/app/admin/settings/actions.ts` | Server Actions לעדכון הגדרות |

#### זרימת חיוב

```
1. משתמש נרשם → 7 ימי נסיון (מוגדר ב-platform_settings)
2. לאחר הנסיון → בחירת מסלול:
   - תדמית: ₪299 + 18% מע"מ = ₪352.82
   - קוויק שופ: ₪399 + 18% מע"מ = ₪470.82
3. עמלות עסקאות: 0.5% מכל עסקה + 18% מע"מ
4. תוספים: מחיר חודשי + ימי נסיון כמוגדר ב-plugin_pricing
```

#### הסליקה

הסליקה מתבצעת דרך **PayPlus** והיא כבר מחוברת למערכת. המחירים נקראים דינמית מהמסד נתונים.
