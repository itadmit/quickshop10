...
export function StoryViewer({
  ...
  const content = (
    ...
    >
      {/* Navigation arrows (desktop) - OUTSIDE the phone frame */}
      {currentIndex > 0 && (
        <button
          onClick={goToPrevious}
          className="absolute right-4 md:right-[calc(50%-220px)] top-1/2 -translate-y-1/2 z-[60] w-12 h-12 hidden md:flex items-center justify-center rounded-full bg-white/20 text-white hover:bg-white/30 transition-colors cursor-pointer"
        >
      ...
      {currentIndex < stories.length - 1 && (
        <button
          onClick={goToNext}
          className="absolute left-4 md:left-[calc(50%-220px)] top-1/2 -translate-y-1/2 z-[60] w-12 h-12 hidden md:flex items-center justify-center rounded-full bg-white/20 text-white hover:bg-white/30 transition-colors cursor-pointer"
        >
          <ChevronLeft className="w-8 h-8" />
        </button>
      )}
      
      {/* Phone Frame Container - Full screen on mobile, phone-sized on desktop */}
      <div className="w-full h-full md:w-[375px] md:h-[90vh] md:max-h-[812px] md:rounded-2xl relative overflow-hidden bg-black">
        {/* Progress bars */}
        ...
        {/* Navigation areas (left/right sides) - for tap navigation */}
        <button
          onClick={goToPrevious}
