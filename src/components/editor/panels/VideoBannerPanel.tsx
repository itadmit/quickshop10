...
/**
 * VideoBannerPanel - Settings panel for video banner section
 * פאנל הגדרות לסקשן באנר וידאו
 */
...
interface VideoBannerPanelProps {
  section: Section;
...
export function VideoBannerPanel({ section, onUpdate, onClose, onDelete }: VideoBannerPanelProps) {
      ...
      <div className="flex flex-col h-full bg-[var(--editor-bg-primary)]" dir="rtl">
        ...
        <div className="px-4 py-3 border-b border-[var(--editor-border-default)] bg-[var(--editor-bg-secondary)]">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-medium text-[var(--editor-text-primary)]">באנר וידאו</h3>
          </div>
            ...
            <div className="space-y-0">
              <MiniAccordion title="וידאו" defaultOpen={true}>
                <div className="space-y-3">
                  <EditorInput label="URL וידאו (MP4)" value={(section.content.videoUrl as string) || ''} onChange={(v) => updateContent('videoUrl', v)} placeholder="https://..." />
                  <EditorInput label="תמונת פולבק" value={(section.content.posterUrl as string) || ''} onChange={(v) => updateContent('posterUrl', v)} placeholder="https://..." />
                  <EditorToggle label="השמעה אוטומטית" value={(section.content.autoplay as boolean) ?? true} onChange={(v) => updateContent('autoplay', v)} />
                  <EditorToggle label="לופ" value={(section.content.loop as boolean) ?? true} onChange={(v) => updateContent('loop', v)} />
                  <EditorToggle label="מושתק" value={(section.content.muted as boolean) ?? true} onChange={(v) => updateContent('muted', v)} />
                </div>
              </MiniAccordion>
            ...
            <div className="space-y-0">
              <MiniAccordion title="פריסה" defaultOpen={true}>
                <div className="space-y-4">
                  ...
                  <EditorSlider label="גובה מינימום" value={(section.settings.minHeight as number) || 500} onChange={(v) => updateSettings('minHeight', v)} min={300} max={800} />
                  <EditorSlider label="שקיפות שכבה" value={((section.settings.overlay as number) || 0.4) * 100} onChange={(v) => updateSettings('overlay', v / 100)} min={0} max={80} step={5} />
                </div>
              </MiniAccordion>

              <MiniAccordion title="כותרת" defaultOpen={false}>
                <TypographyControl label="" prefix="title" settings={section.settings} onChange={updateSettings} defaultSize={48} defaultSizeMobile={32} defaultColor="#ffffff" defaultWeight="bold" />
              </MiniAccordion>
