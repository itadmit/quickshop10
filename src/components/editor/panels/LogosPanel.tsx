...
/**
 * LogosPanel - Settings panel for logos section
 * פאנל הגדרות לסקשן לוגואים
 */
...
export function LogosPanel({ section, onUpdate, onClose, onDelete }: LogosPanelProps) {
  ...
  const logos = (section.content.logos as Array<{url: string; alt?: string; link?: string}>) || [];
  ...
  const addLogo = () => {
    updateContent('logos', [...logos, { url: '', alt: '' }]);
  };
                  ...
                  {logos.map((logo, index) => (
                    <div key={index} className="p-3 bg-[var(--editor-bg-tertiary)] rounded-lg space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-medium">לוגו {index + 1}</span>
                        <button onClick={() => removeLogo(index)} className="text-red-500 text-xs hover:text-red-600">מחק</button>
                      </div>
                      <EditorInput label="URL תמונה" value={logo.url} onChange={(v) => updateLogo(index, 'url', v)} placeholder="https://..." />
                      <EditorInput label="תיאור (alt)" value={logo.alt || ''} onChange={(v) => updateLogo(index, 'alt', v)} />
                      <EditorInput label="קישור (אופציונלי)" value={logo.link || ''} onChange={(v) => updateLogo(index, 'link', v)} />
                    </div>
                  ))}
                  <button onClick={addLogo} className="w-full py-2 text-xs text-blue-600 border border-dashed border-blue-300 rounded-lg hover:bg-blue-50">+ הוסף לוגו</button>
                </div>
              </MiniAccordion>
            </div>
          )}
          ...
          {activeTab === 'design' && (
            <div className="space-y-0">
              <MiniAccordion title="פריסה" defaultOpen={true}>
                <div className="space-y-4">
                  <EditorSlider label="לוגואים בשורה (מחשב)" value={(section.settings.columns as number) || 6} onChange={(v) => updateSettings('columns', v)} min={3} max={8} step={1} />
                  <EditorSlider label="לוגואים בשורה (מובייל)" value={(section.settings.mobileColumns as number) || 3} onChange={(v) => updateSettings('mobileColumns', v)} min={2} max={4} step={1} />
                  <EditorSlider label="גודל לוגו" value={(section.settings.logoSize as number) || 80} onChange={(v) => updateSettings('logoSize', v)} min={40} max={150} />
              ...
              <MiniAccordion title="כותרת" defaultOpen={false}>
                <TypographyControl label="" prefix="title" settings={section.settings} onChange={updateSettings} defaultSize={24} defaultSizeMobile={20} defaultColor="#6b7280" defaultWeight="normal" />
              </MiniAccordion>
            </div>
          )}
