...
/**
 * SplitBannerPanel - Settings panel for split banner section
 * פאנל הגדרות לסקשן באנר מפוצל
 */
...
interface SplitBannerPanelProps {
  section: Section;
...
export function SplitBannerPanel({ section, onUpdate, onClose, onDelete }: SplitBannerPanelProps) {
              ...
              <MiniAccordion title="טקסט" defaultOpen={true}>
                <div className="space-y-3">
                  ...
                  <div className="space-y-1">
                    <label className="text-xs text-[var(--editor-text-secondary)]">תוכן</label>
                    <textarea
              ...
              <MiniAccordion title="פריסה" defaultOpen={true}>
                <div className="space-y-4">
                  <EditorSelect
                    ...
                    options={[
                      { value: 'right', label: 'תמונה מימין' },
                      { value: 'left', label: 'תמונה משמאל' },
                    ]}
                  />
                  <EditorSlider label="יחס תמונה/טקסט" value={(section.settings.imageRatio as number) || 50} onChange={(v) => updateSettings('imageRatio', v)} min={30} max={70} />
                  <AlignmentControl settings={section.settings} onChange={updateSettings} label="יישור טקסט" />
                </div>
              </MiniAccordion>

              <MiniAccordion title="צבעים" defaultOpen={false}>
                <div className="space-y-3">
                  <EditorColorPicker label="רקע טקסט" value={(section.settings.backgroundColor as string) || '#1f2937'} onChange={(v) => updateSettings('backgroundColor', v)} />
                  <EditorSlider label="שקיפות תמונה" value={((section.settings.imageOverlay as number) || 0) * 100} onChange={(v) => updateSettings('imageOverlay', v / 100)} min={0} max={50} step={5} />
                </div>
              </MiniAccordion>

              <MiniAccordion title="כותרת" defaultOpen={false}>
                <TypographyControl label="" prefix="title" settings={section.settings} onChange={updateSettings} defaultSize={36} defaultSizeMobile={28} defaultColor="#ffffff" defaultWeight="bold" />
              </MiniAccordion>
