  ...
  try {
    ...
    if (!response.success) {
      ...
      if (alreadyExists && existingShipmentNumber) {
        console.log(`[SendShipment] Shipment already exists: ${existingShipmentNumber}`);
        ...
        const [shipment] = await db.insert(shipments).values({
          ...
          trackingNumber: existingShipmentNumber,
          status: 'created',
          statusDescription: 'נשלח ידנית (משלוח קיים)',
          labelUrl,
        ...
        console.log(`[SendShipment] Success - Saved existing shipment ${existingShipmentNumber}`);
