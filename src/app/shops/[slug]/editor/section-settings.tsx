...
function ContentSettings({ 
  ...
  return (
    <div className="p-4 space-y-6">
      ...
      {section.type === 'google_reviews' && (
        <GoogleReviewsContentSettings section={section} onUpdate={onUpdate} storeInfo={storeInfo} />
      )}
...
function DesignSettings({ 
  ...
  // Google Reviews design settings
  if (section.type === 'google_reviews') {
    return (
      <div className="p-4 space-y-6">
        <SettingsGroup title="פריסה">
          <SelectField
            label="פריטים בתצוגה (מחשב)"
            value={String((section.settings.columns as number) || 3)}
            options={[
              { value: '2', label: '2' },
              { value: '3', label: '3' },
        ...
        <SettingsGroup title="גודל כותרת">
          <SliderField
            label="גודל (מחשב)"
            value={(section.settings.titleSize as number) || 30}
            min={16}
            max={60}
            suffix="px"
            onChange={(v) => updateSettings('titleSize', v)}
          ...
          <SliderField
            label="גודל (מובייל)"
            value={(section.settings.titleSizeMobile as number) || 24}
            min={14}
            max={40}
            suffix="px"
            onChange={(v) => updateSettings('titleSizeMobile', v)}
          />
        </SettingsGroup>
        ...
        <SettingsGroup title="גובה ומרווחים">
          <SliderField
            label="גובה מינימום"
            value={(section.settings.minHeight as number) || 0}
            min={0}
            max={600}
            suffix="px"
            onChange={(v) => updateSettings('minHeight', v)}
          />
          <SliderField
            label="ריווח עליון"
            value={(section.settings.marginTop as number) || 0}
...
// Google Reviews Content Settings
function GoogleReviewsContentSettings({ section, onUpdate, storeInfo }: { section: Section; onUpdate: (updates: Partial<Section>) => void; storeInfo?: StoreInfo }) {
  const [isConnecting, setIsConnecting] = useState(false);
  const isConnected = !!(section.content.googleAccountId);
  const businessName = (section.content.businessName as string) || '';
  const totalReviews = (section.content.totalReviews as number) || 0;
  const averageRating = (section.content.averageRating as number) || 0;
  ...
  const handleConnect = () => {
    ...
    const popup = window.open(
      `/shops/${storeInfo?.slug}/admin/settings/integrations/google-business/connect`,
      'google-auth',
  ...
  return (
    <>
      <SettingsGroup title="חיבור לגוגל">
        {isConnected ? (
          <div className="space-y-3">
            {/* Connected State */}
          ...
          <div className="space-y-3">
            ...
            <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg">
              <p className="text-xs text-amber-700">
                <strong>💡 טיפ:</strong> הזן את פרטי העסק והביקורות שלך מגוגל.
              </p>
            </div>
          </div>
        )}
