...
// ============================================
// Live Preview - Center Panel - עברית
// ============================================
...
export function LivePreview({
  ...
  return (
    <div className={`bg-white shadow-2xl rounded-lg overflow-hidden ${getPreviewWidth()}`}>
      ...
      {/* Preview Content */}
      <div className="overflow-auto max-h-[calc(100vh-200px)]" dir="rtl">
        {/* Header Preview */}
        <header className="sticky top-0 z-50 bg-white border-b border-gray-100">
          <div className="px-6 py-4 flex items-center justify-between">
            <div className="flex items-center gap-6">
              ...
              {previewMode !== 'mobile' && (
                <nav className="flex gap-4 text-xs uppercase tracking-wider">
                  <span className="text-gray-600 hover:text-black cursor-pointer">אודות</span>
                  <span className="text-gray-600 hover:text-black cursor-pointer">חנות</span>
                </nav>
              )}
            </div>
            ...
            <div className="flex items-center gap-4">
              {previewMode !== 'mobile' && (
                <span className="text-xs text-gray-500">עברית</span>
              )}
        ...
        {sections.filter(s => s.isActive).map((section) => (
          ...
          >
            {/* Selection Indicator */}
            {selectedSectionId === section.id && (
              <div className="absolute top-0 right-0 z-10 px-2 py-1 bg-blue-500 text-white text-[10px] font-medium rounded-bl">
            ...
            {selectedSectionId === section.id && (
              <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-10 flex items-center gap-1 bg-gray-900/90 rounded-full px-2 py-1 shadow-lg">
                <button className="p-1.5 hover:bg-white/10 rounded-full transition-colors" title="הוסף">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2">
                    <path d="M12 5v14M5 12h14" />
                  </svg>
                </button>
                <button className="p-1.5 hover:bg-white/10 rounded-full transition-colors" title="הזז">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2">
                    <path d="M5 9l4-4 4 4M5 15l4 4 4-4" />
                  </svg>
                </button>
                <button className="p-1.5 hover:bg-white/10 rounded-full transition-colors" title="מחק">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2">
    ...
    default:
      return (
        <div className="py-20 text-center bg-gray-50">
          <p className="text-gray-400">סוג סקשן לא מוכר: {section.type}</p>
        </div>
      );
  }
}
    ...
    return (
      <div className="grid grid-cols-2" style={{ height: settings.height || '50vh' }}>
        <div className="bg-gray-200 flex items-center justify-center">
          <span className="text-gray-400 text-sm">הוסף תמונה שמאלית</span>
        </div>
        <div className="bg-gray-300 flex items-center justify-center">
          <span className="text-gray-400 text-sm">הוסף תמונה ימנית</span>
        </div>
      </div>
    );
  }
