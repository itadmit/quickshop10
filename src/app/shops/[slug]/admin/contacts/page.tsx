...
export default async function ContactsPage({ params, searchParams }: ContactsPageProps) {
  ...
  const [
    ...
    // לקוחות = customers with at least 1 order
    db.select({ count: count() }).from(customers).where(and(eq(customers.storeId, store.id), gt(customers.totalOrders, 0))),
  ...
  if (type === 'customer') {
    ...
    const tabs: Tab[] = [
      ...
      { id: 'customer', label: 'לקוחות', count: customersCount[0].count },
      { id: 'newsletter', label: 'ניוזלטר', count: newsletterCount[0].count },
      { id: 'club_member', label: 'מועדון', count: clubMemberCount[0].count },
