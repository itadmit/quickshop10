          ...
          <div className="md:col-span-1">
            <div className="bg-white border border-gray-200 rounded-lg p-6 sticky top-24">
              ...
              <div className="space-y-3 text-sm">
                ...
                {((order.discountDetails as Array<{type: 'coupon' | 'auto' | 'gift_card' | 'credit' | 'member' | 'loyalty_tier'; code?: string; name: string; description?: string; amount: number}>) || []).map((discount, idx) => (
                  <div 
                    key={idx} 
                    className={`flex justify-between ${
                      discount.type === 'gift_card' ? 'text-purple-600' :
                      discount.type === 'credit' ? 'text-blue-600' :
                      discount.type === 'loyalty_tier' ? 'text-violet-600' :
                      'text-green-600'
                    }`}
                  >
                    <span className="flex items-center gap-1">
                      ...
                      {discount.type === 'coupon' ? `קופון ${discount.code}${discount.description ? ` (${discount.description})` : ''}` :
                       ...
                       discount.type === 'loyalty_tier' ? `הנחת ${discount.name}${discount.description ? ` (${discount.description})` : ''}` :
